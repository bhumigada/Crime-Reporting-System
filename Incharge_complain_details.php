...
<body>
  ...
  <div class="container">
    ...
    $result1 = $conn->query("SELECT * FROM `police_station` WHERE i_id='$i_id'");
    // $q2=mysqli_fetch_assoc($result1);
    $location = $result1->fetch_assoc()['location'];
    echo "Hello $location";
...
// echo "$location";
    ...
    if(isset($_POST['assign']))
    {
      if($_SERVER["REQUEST_METHOD"]=="POST")
      {
        ...
        $res1=$conn->query("SELECT p_id FROM police where p_name='$pname'");
        $q3=mysqli_fetch_assoc($res1);
        $pid=$q3['p_id'];
      
      
        $res=$conn->query("update complaint set inc_status='Assigned',pol_status='In Process',p_id='$pid' where c_id='$cid'");
...
<select class="form-control" name="police_name" style="margin-left:40%; width:250px;">
            <?php
                        $p_name=$conn->query("select p_name from police where location='$location'");
